...
and then to stabilise the variance of the data we could use a square root transform, but we follow Greenacre *et al* and use a quarter root transform

```{r}
baxter_qroot <- baxter_prop^0.25
```

The aim of this brief example is to explore the effects of `dx` and `std_Age` on the OTU composition. We can fit this model using
