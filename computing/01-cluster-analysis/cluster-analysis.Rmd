...
The functions `hclust()` (in package
`mva`) and `agnes()` (in package `cluster`) are the two standard functions for
...
containing just the standardised water chemistry data. As it's
...
frame or matrix.
```{r}
pondsenv <- subset(ponds, select = pH:Maxdept)
...
be given as arguments. The resulting
`pond.slink` object contains the results
...
resulting _k_ clusters on the dendrogram. Finally, the cophenetic
correlation is calculated, using the `cophenetic()`
function to first generate the cophenetic distances between ponds from the
dendrogram\footnote{The distances are stored in `coph.slink` for later
...
```{r}
v_url <- ""
vltava <- read.delim (v_url, row.names = 1)
```