...
# Compositional data &mdash; ALR transformations

$$
\text{ALR}(j) | \text{ref} = \log(x_j / x_{\text{ref}}), \; j \in {1, \dots, D}
$$

There are $D$ possible ALR transformations, depending on the choice of *reference* composition / part

Choose one of the variables in the simplex as the reference part against which all other variables are normalised

The choice of the reference part can affect the success of the transformation; choose a reference part that varies little across the samples

In wide data, $p \gg n$, it is often the case that a suitable low-variance reference can be found

---

# Compositional data &mdash; CLR transformations
